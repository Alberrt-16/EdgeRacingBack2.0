...
{ 
    ...
    foreach ($races as $race) {
        $drivers = Driver::where('competition_id', $race->competition_id)->get(); // Mezclar para aleatoriedad
        $positionCounter = 1;
    
        foreach ($drivers as $driver) {
            $didNotFinish = fake()->boolean(10); // 10% chance de DNF
            $isFastLap = fake()->boolean(5);     // 5% chance de vuelta rápida
            $points = $didNotFinish ? 0 : max(0, 25 - (($positionCounter - 1) * 2));
    
            $ranking = Ranking::create([
                'race_id' => $race->id,
                'competition_id' => $race->competition_id,
                'driver_id' => $driver->id,
                'position' => $didNotFinish ? null : $positionCounter,
                'points' => $points,
                'fastLap' => $isFastLap,
            ...
            if (!$didNotFinish) {
                $positionCounter++; // Solo incrementa si el piloto terminó la carrera
            }
